(function(global) {
   ...
   */
  function Pig(imageData, options, pswpElement, pswpItems, pswpUI) {
    // Global State
    this.pswpElement = pswpElement;
    this.pswpItems = pswpItems;  //TODO: Just have a single list for grid and lightbox images
    this.pswpUI = pswpUI;
    this.inRAF = false;
  ...
  function ProgressiveImage(singleImageData, index, pig) {
    ...
    this.imageSizes = singleImageData.imageSizes;  // Map of size to image URL
    this.aspectRatio = singleImageData.aspectRatio;  // Aspect Ratio
    this.metadata = singleImageData.metadata;  // Additional user-defined data
  ...
  ProgressiveImage.prototype.load = function() {
    ...
    setTimeout(function() {
      ...
      // Show super low-res thumbnail during loading
      if (!this.thumbnail) {
        this.thumbnail = new Image();
        this.thumbnail.src = this.imageSizes[this.pig.settings.thumbnailSize];
        ...
        this.getElement().insertBefore(this.thumbnail, this.caption);
      }

      // Show full image
      if (!this.fullImage) {
        this.fullImage = new Image();
        this.fullImage.src = this.imageSizes[this.pig.settings.getImageSize(this.pig.lastWindowWidth)];
        this.fullImage.onload = function() {
          ...
          // Show the caption when the image finishes loading
          if (this.caption) {
              this.caption.style.visibility = "visible";
          }
        }.bind(this);

        this.getElement().insertBefore(this.fullImage, this.caption);
      }
    }.bind(this), 100);
  };
  ...
  ProgressiveImage.prototype.getElement = function() {
    if (!this.element) {
      this.element = this.createHtmlElements();
      this._updateStyles();
  ...
  /**
   * Creates the HTML elements corresponding to this image, and inserts them
   * into the DOM. Returns the figure element that holds everything.
   *
   *   <figure class="pig-figure" style="transform: ...">
   *     <img class="pig-thumbnail pig-loaded" src="/path/to/thumbnail/image.jpg" />
   *     <img class="pig-loaded" src="/path/to/500px/image.jpg" />
   *     <div class="dir-tile-info">
   *       <div class="dir-tile-info-icon"></div>
   *       <div class="dir-tile-info-text">
   *         <p class="dir-tile-info-text">Directory name or filename</p>
   *       </div>
   *     </div>
   *   </figure>
   *
   * @returns {HTMLElement} The DOM element associated with this instance.
   */
  ProgressiveImage.prototype.createHtmlElements = function() {
      var figure = document.createElement(this.pig.settings.figureTagName);
      ...
      if (this.metadata.type == "dir") {
          tileInfoClass = "dir-tile-info";
          tileTextClass = "dir-tile-info-text";
          tileIconClass = "dir-tile-info-icon";
      } else {
          tileInfoClass = "photo-tile-info";
          tileTextClass = "photo-tile-info-text";
          tileIconClass = "photo-tile-info-icon";
      }

      var tileInfoDiv = document.createElement("div");
      tileInfoDiv.className = tileInfoClass;
      tileInfoDiv.style.visibility = "hidden";  // hide initally, show when image is done loading

      var tileInfoIconSpan = document.createElement("span");
      tileInfoIconSpan.className = tileIconClass;
      var iconGlyph = document.createElement("i");
      iconGlyph.className = "far fa-images";
      tileInfoIconSpan.appendChild(iconGlyph);
      tileInfoDiv.appendChild(tileInfoIconSpan);

      var tileInfoTextDiv = document.createElement("div");
      tileInfoTextDiv.className = tileTextClass;
      var tileInfoTextP = document.createElement("p");
      tileInfoTextP.className = tileTextClass;
      tileInfoTextP.innerHTML = this.metadata.name;
      tileInfoTextDiv.appendChild(tileInfoTextP);
      tileInfoDiv.appendChild(tileInfoTextDiv);

      figure.appendChild(tileInfoDiv);

      this.caption = tileInfoDiv;
  ...
  /**
   * Updates the event handlers for a single image:
   * - If a standard image is clicked, opens the lightbox and points it to
   *   the index of the clicked image.
   * - If a directory is clicked, redirects to a new page to load the grid
   *   for that directory.
   */
  ProgressiveImage.prototype._updateEventHandlers = function() {
    if (this.metadata.type == 'dir') {
        // Directory, redirect to the new location
        var url = this.metadata.url;
        this.getElement().addEventListener('click', function() {
            window.location.href = url;
        });
    } else {
        // Just a regular image, open the lightbox
        var options = {
            index: this.metadata.lightboxIndex,
            history: true,
            galleryPIDs: true,
        }
        var pswpElement = this.pig.pswpElement;
        var pswpUI = this.pig.pswpUI;
        var pswpItems = this.pig.pswpItems;
